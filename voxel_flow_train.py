...
def train(dataset_frame1, dataset_frame2, dataset_frame3):
  ...
  with tf.Graph().as_default():
    ...
    summaries.append(tf.compat.v1.summary.scalar('reproduction_loss', reproduction_loss))
    ...
    summaries.append(tf.compat.v1.summary.image('Input Image', input_placeholder, 3))
    summaries.append(tf.compat.v1.summary.image('Output Image', prediction, 3))
    summaries.append(tf.compat.v1.summary.image('Target Image', target_placeholder, 3))
    ...
    #session_config = tf.compat.v1.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))
    ...
    data_size = len(data_list_frame1)   # 276917
    epoch_num = int(data_size / FLAGS.batch_size)   # FLAGS.batch_size = 32
    ...
    for step in range(190000, FLAGS.max_steps):
      ...
      #print(len(batch_data_list_frame1))
      #print(dataset_frame1.process_func(batch_data_list_frame1[0]).shape)
      #exit()
      # Load batch data.
...
def test(dataset_frame1, dataset_frame2, dataset_frame3):
  ...
  with tf.Graph().as_default():
    ...
    saver = tf.compat.v1.train.Saver(tf.all_variables())
    #session_config = tf.compat.v1.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.5, allow_growth=True))
    #sess = tf.Session(config = session_config)
    ##sess = tf.Session()
    session_config = tf.compat.v1.ConfigProto()
    session_config.gpu_options.allow_growth = True
